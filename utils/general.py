...
def non_max_suppression(
    ...
    _prediction = deepcopy(prediction)
    _prediction[..., :4] = xywh2xyxy(prediction[..., :4])  # xywh to xyxy
    # prediction[..., :4] = xywh2xyxy(prediction[..., :4])  # xywh to xyxy

    t = time.time()
    # output = [torch.zeros((0, 6 + nm), device=prediction.device)] * bs
    # for xi, x in enumerate(prediction):  # image index, image inference
    output = [torch.zeros((0, 6 + nm), device=_prediction.device)] * bs
    for xi, x in enumerate(_prediction):  # image index, image inference
        # Apply constraints
